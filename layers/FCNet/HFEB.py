...
class Mlp(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Conv2d(in_features, hidden_features, 1)  # 1x1 conv
        self.dwconv = DWConv(hidden_features)  # CFF: Convolutional feed-forward network
        self.act = act_layer()  # GELU
        self.fc2 = nn.Conv2d(hidden_features, out_features, 1)  # 1x1 conv
...
class AttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_atten = self.conv1(attn)  # 1x1 conv
        # append a se operation
        b, c, _, _ = x.size()
        time_atten = self.avg_pool(x).view(b, c)
        time_atten = self.fc(time_atten).view(b, c, 1, 1)
        return time_atten * spatial_atten * u
